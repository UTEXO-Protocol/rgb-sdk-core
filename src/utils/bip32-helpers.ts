...
/** Normalize seed to Uint8Array for BIP32 operations */
export function normalizeSeedBuffer(seed: BufferLike): Uint8Array {
  if (!seed) {
  ...
  if (seed instanceof Uint8Array) {
    ...
    if ('buffer' in seed && seed.buffer) {
      ...
      if (bufferValue instanceof ArrayBuffer) {
        ...
        return new Uint8Array(bufferValue, byteOffset, byteLength);
      } else {
        try {
          return new Uint8Array(seed as ArrayLike<number>);
        } catch (error) {
